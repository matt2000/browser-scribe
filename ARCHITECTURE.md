...
| `keyup` | Keyboard input | All keyup events |
| `submit` | Form submission with field values | All submits, excludes passwords |
| `change` | Form field changes | Form elements only, hides passwords |
| `dom_mutation` | New elements added to DOM | Batched mutations |
| `xhr_response` | XHR request response | Within 5s of user action |
| `fetch_response` | Fetch request response | Within 5s of user action |
...
1. **Password Protection**: All password fields are explicitly excluded from logging
